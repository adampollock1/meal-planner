...
interface DatePickerProps {
  ...
  placeholder?: string;
  inline?: boolean; // Show calendar inline without toggle button
}

const DAY_LABELS_SUNDAY = ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'];
const DAY_LABELS_MONDAY = ['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su'];

export function DatePicker({ selectedDate, onSelectDate, placeholder = 'Select date', inline = false }: DatePickerProps) {
  ...
  // Close on click outside (only for non-inline mode)
  useEffect(() => {
    if (inline) return;
  ...
  const calendarContent = (
    <div className={inline ? "bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-200/50 dark:border-slate-700/50 p-4" : "bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl rounded-xl shadow-xl border border-slate-200/50 dark:border-slate-700/50 p-4 min-w-[280px] animate-scale-in"}>
      {/* Month navigation */}
  ...
  // Inline mode - just render the calendar
  if (inline) {
    return calendarContent;
  }
  
  // Dropdown mode - toggle button with calendar popup
  return (
